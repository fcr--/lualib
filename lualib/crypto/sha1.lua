...
return function(message, binary)
  ...
  local last_chunks = table.concat {
  ...
  if binary then
    return table.concat { encode_word(h0), encode_word(h1), encode_word(h2), encode_word(h3), encode_word(h4) }
  else
    return table.concat { tohex(h0), tohex(h1), tohex(h2), tohex(h3), tohex(h4) }
  end
end
